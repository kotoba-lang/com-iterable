(ns iterable.main
  "Kotodama WASM entrypoint for the Iterable clean-room actor (L4) — Clojure port.
  ...
  facts (`iterable.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "iterable")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"iterable.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "iterable-compat" :tier tier :entities entities} 200])
...
(defn start! [] :iterable-compat/ready)
